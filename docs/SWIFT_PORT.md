...
| PBR (Cook-Torrance GGX) | ✅ BackroomsRender | ⬜ compile-only | GGX/Smith/Schlick replacing Blinn-Phong, energy-conserving diffuse, 0.04 dielectric F0. Diffuse is deliberately **not** divided by pi — the level light intensities were authored against the old non-normalised model |
| HDR + bloom + tone-map | ✅ BackroomsRender | ⬜ compile-only | scene renders to `rgba16Float` in linear light; ACES moved out of the scene shader into a composite pass, with a quarter-res bright pass and separable blur. `misc.y` picks who tone-maps, since the direct-to-drawable fallback has no composite downstream |
| SSAO | ✅ BackroomsRender | ⬜ compile-only | half-res hemisphere AO, normals from depth derivatives (no G-buffer in a forward renderer). Supersedes `contactAO`, which darkens by height above the floor and cannot know about a corner |
| Volumetric inscatter | ✅ BackroomsRender | ⬜ compile-only | half-res 16-step march, lamp cone + 4 nearest point lights, capped at 22m. **Unprofiled** — the first thing to turn down if frame time suffers |
| Water (Poolrooms) | ✅ BackroomsRender | ⬜ compile-only | alpha-blended sheet emitted **per water cell** rather than one plane under the whole floor — with depth-write off, a plane running beneath every platform hazes through the tile. Dual scrolling procedural normals, Fresnel-driven opacity, GGX specular off the lamp |
| Shadows | ⬜ | — | **Not the lamp**: it originates at the camera, and a light co-located with the eye casts no shadow anyone can see. The shadows worth having are from the ceiling fixtures — spot-style 2D maps pointing down, one geometry pass per shadowed light, so realistically the nearest one or two |
